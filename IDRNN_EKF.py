...
class IDRNN(object):
    ...
    def build_model(self):
        ...
        self.sim_fn = theano.function([x_in, x_drive], outputs=y, givens=[(H, self.h_init)])
        self.pred_cost = theano.function([x_in,x_drive, y_out], outputs=cost, givens=[(H, self.h_init)]) 
    ...
    def train(self, SEED):
        ...
        for epochs_index in xrange(self.n_epochs) :  
            ...
            if numpy.mod(epochs_index+1, self.valid_fre) == 0:
                train_err = self.pred_cost(train_data[:-1,0], train_data[self.n_input:,1], train_data[self.n_input:,-1]) / train_data.shape[0]
                test_err  = self.pred_cost(test_data[:-1,0],   test_data[self.n_input:,1], test_data[ self.n_input:,-1])   / test_data.shape[0]
                valid_err = self.pred_cost(valid_data[:-1,0], valid_data[self.n_input:,1], valid_data[self.n_input:,-1]) / valid_data.shape[0]
        ...
        for i in numpy.arange(self.n_predict):
            y_predict[i] = self.sim_fn(x_train_end, test_data[i:i+1,1])
...
if __name__ == '__main__':
    ...
    rnn = IDRNN(n_input=n_input, n_hidden=n_hidden, n_output=n_output, 
