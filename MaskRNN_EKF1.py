...
f_train = theano.function([x, x_mask, t], [cost, h_tmp[-1]], updates=update_W,
                                name='EKF_f_train',
                                mode=compile_mode,
...
for epochs_index in xrange(n_epochs) :  
    ...
    for batch_index, train_index in kf:
        ...
        _mask = _mask[:-1]
        train_err, h_init_continue = f_train(_x, _mask, _y)
        for i in numpy.arange(n_segment_h):
            h_init[i].set_value(numpy.random.normal(size=(n_hidden/n_segment_h,),loc=0,scale=0.1))
