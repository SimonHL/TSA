...
def step(*args):
    ...
    for i in xrange(n_segment_h):
        ...
        elif build_method == 1 or build_method == 2:   # 方式1 和方式2 都需要三角化
            h_tmp = T.dot(W_hid[i*n_segment_h + i],hid_taps[i]) # 下标从i开始，可以使其成为上三角矩阵
            for j in xrange(i+1,n_segment_h):  
                h_tmp += T.dot(W_hid[i*n_segment_h + j],hid_taps[j])
        elif build_method == 3:
            h_tmp = T.dot(W_hid[i*n_segment_h + i],hid_taps[i]) # 只使用对角部分
        else:
            raise ValueError
...
build_method = 0  # 0: blockRNN  1: CW-RNN 2: MaskRNN 3: DialogRNN
...
saveto = 'MaskRNN_b{}_i{}_h{}_nh{}_S{}.npz'.format(
          build_method, init_method, n_hidden, n_segment_h, SEED)
...
elif build_method == 1 or build_method == 2:   #回归部分，左下部分不参与计算
    for i in xrange(n_segment_h):
        for j in xrange(i,n_segment_h):
            params4grad.extend([W_hid[i*n_segment_h+j]])
elif build_method == 3:
    for i in xrange(n_segment_h):
        params4grad.extend([W_hid[i*n_segment_h+i]])
else:
    raise ValueError
...
train_output_info = []
train_output_info.append(cost)
for i in h_tmp:
    train_output_info.append(i[-1]) 

f_train = theano.function([x, x_mask, t], train_output_info, updates=update_W,
                                name='EKF_f_train',
                                mode=compile_mode,
...
for epochs_index in xrange(n_epochs) :  
    ...
    for batch_index, train_index in kf:
        ...
        _mask = _mask[:-1]
        tmp_result = f_train(_x, _mask, _y)
        train_err = tmp_result[0]
        h_init_continue  = [tmp_result[i] for i in xrange(1,len(tmp_result))]
        for i in numpy.arange(len(h_init_continue)):
            h_init[i].set_value(h_init_continue[i])
...
plt.plot( range(index_train_end, index_valid_end), y_sim[index_train_end-index_start:index_valid_end-index_start], 'y')
plt.plot( range(index_valid_end, index_test_end),  y_sim[index_valid_end-index_start:index_test_end-index_start], 'b')
plt.plot( range(data_y.shape[0]), data_y,'k')  # 原始信号
